"""Export object-detection-only results for one video or a folder of videos."""
...
from realtime_od.model_registry import DEFAULT_MODEL_KEY, MODEL_REGISTRY, get_model_spec
...
def parse_args() -> argparse.Namespace:
    ...
    source_group.add_argument("--source", help="Single video/webcam source to export.")
    source_group.add_argument("--input-dir", default="video", help="Folder of videos to export. Used when --source is omitted.")
    parser.add_argument("--output-root", default="outputs/detection")
    parser.add_argument("--output-dir", help="Explicit output folder. Only valid with --source.")
    parser.add_argument("--model", choices=sorted(MODEL_REGISTRY), default=DEFAULT_MODEL_KEY)
    parser.add_argument("--weights", help="Override checkpoint path. If omitted, --model selects a VisDrone checkpoint.")
    ...
    if args.output_dir and not args.source:
        parser.error("--output-dir is only valid with --source")
    return args


def iter_sources(args: argparse.Namespace) -> list[str]:
    if args.source:
        return [args.source]
...
def export_one(source: str, args: argparse.Namespace, index: int, total: int) -> None:
    model_spec = get_model_spec(args.model)
    weights = args.weights or model_spec.weights
    output_dir = Path(args.output_dir) if args.output_dir else Path(args.output_root) / safe_stem(source)
    ...
    print(f"[{index}/{total}] Exporting detection for {source}")
    ...
    print(f"Model: {model_spec.label}")
...
def main() -> None:
    ...
    for index, source in enumerate(sources, start=1):
        export_one(source, args, index, len(sources))
